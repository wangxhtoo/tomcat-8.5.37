...
public class Digester extends DefaultHandler2 {
	...
	/**
	 * Stack whose elements are List objects, each containing a list of Rule objects
	 * as returned from Rules.getMatch(). As each xml element in the input is
	 * entered, the matching rules are pushed onto this stack. After the end tag is
	 * reached, the matches are popped again. The depth of is stack is therefore
	 * exactly the same as the current "nesting" level of the input xml.
	 *
	 * @since 1.6
	 */
	...
	/**
	 * Registered namespaces we are currently processing. The key is the namespace
	 * prefix that was declared in the document. The value is an ArrayStack of the
	 * namespace URIs this prefix has been mapped to -- the top Stack element is the
	 * most current one. (This architecture is required because documents can
	 * declare nested uses of the same prefix for different Namespace URIs).
	 */
	 ...
	 * established before the first rule is added, a default implementation will be
	 * provided.
	 */
	...
	/**
	 * Return the currently mapped namespace URI for the specified prefix, if any;
	 * otherwise return <code>null</code>. These mappings come and go dynamically as
	...
	/**
	 * Return the class loader to be used for instantiating application objects when
	 * required. This is determined based upon the following rules:
	 * <ul>
	...
	/**
	 * Set the class loader to be used for instantiating application objects when
	 * required.
	 *
	 * @param classLoader The new class loader to use, or <code>null</code> to
	 ...
	 * underlying implementation of <code>org.xml.sax.XMLReader</code>. See <a href=
	 ...
	 * <code>getParser()</code> method is called for the first time, either directly
	 * or indirectly.
	 *
	 ...
	 *                                         occurs
	 * @exception SAXNotRecognizedException    if the property name is not
	 *                                         recognized
	 * @exception SAXNotSupportedException     if the property name is recognized
	 *                                         but not supported
	 */
	...
	/**
	 * @return the public identifier of the DTD we are currently parsing under, if
	 *         any.
	 */
	 ...
	 * <code>Rule</code> objects.
	 *
	 * @param ruleNamespaceURI Namespace URI that must match on all subsequently
	 *                         added rules, or <code>null</code> for matching
	 ...
	 * @exception SAXNotSupportedException  if the property name is recognized but
	 *                                      not supported
	 */
	 ...
	 * collection and associated matching policy. If none has been established, a
	 ...
	 * <code>Thread.currentThread().getContextClassLoader()</code>) to resolve/load
	 * classes that are defined in various rules. If not using Context ClassLoader,
	 * then the class-loading defaults to using the calling-class' ClassLoader.
	 *
	 ...
	 *
	 * FIX ME: there is a bug in JAXP/XERCES that prevent the use of a parser that
	 * contains a schema with a DTD.
	 * 
	...
	/**
	 * Process notification of ignorable whitespace received from the body of an XML
	 * element.
	 *
	 * @param buffer The characters from the XML document
	 * @param start  Starting offset into the buffer
	...
	/**
	 * Set the <code>EntityResolver</code> used by SAX when resolving public id and
	 * system id. This must be called before the first call to <code>parse()</code>.
	 * 
	...
	/**
	 * Parse the content of the specified file using this Digester. Returns the root
	 * element from the object stack (if any).
	 *
	...
	/**
	 * Parse the content of the specified input source using this Digester. Returns
	...
	/**
	 * Parse the content of the specified input stream using this Digester. Returns
	 * the root element from the object stack (if any).
	 *
	...
	/**
	 * <p>
	 * Register the specified DTD URL for the specified public identifier. This must
	 * be called before the first call to <code>parse()</code>.
	 * </p>
	 * <p>
	 ...
	 * resource will be loaded). A common use case for this method is to register
	 * local URLs (possibly computed at runtime by a classloader) for DTDs. This
	 * allows the performance advantage of using a local version without having to
	 ...
	 * sophisticated features are required, using {@link #setEntityResolver} to set
	 * a custom resolver is recommended.
	 * </p>
	 * <p>
	 ...
	 *
	 * @param pattern    Element matching pattern
	 * @param paramIndex Zero-relative parameter index to set (from the body of this
	 *                   element)
	 * @see CallParamRule
	 */
	 ...
	 *                               during object creation will be ignored.
	 ...
	 * @param attributeName Attribute name that optionally overrides the default
	 ...
	 * @param paramType  Java class name of the expected parameter type (if you wish
	 *                   to use a primitive type, specify the corresponding Java
	 *                   wrapper class instead, such as
	 ...
	 * <p>
	 * Calling this method <i>might</i> allow another document of the same type to
	 * be correctly parsed. However this method was not intended for this purpose.
	 * In general, a separate Digester object should be created for each document to
	 * be parsed.
	 */
	public void clear() {
	 ...
	 * [getCount()-1] is the bottom element. If the specified index is out of range,
	 * return <code>null</code>.
	 *
	 * @param n Index of the desired element, where 0 is the top of the stack, 1 is
	 *          the next element down, and so on.
	...
	/**
	 * When the Digester is being used as a SAXContentHandler, this method allows
	 * you to access the root object that has been created after parsing.
	 *
	 * @return the root object that has been created after parsing or null if the
	 *         digester has not parsed any XML yet.
	 */
	...
	/**
	 * <p>
	 * Provide a hook for lazy configuration of this <code>Digester</code> instance.
	 * The default implementation does nothing, but subclasses can override as
	 * needed.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * <p>
	 * Return the top object on the parameters stack without removing it. If there
	...
	public SAXException createSAXException(String message, Exception e) {
		...
		if (locator != null) {
			String error = "Error at (" + locator.getLineNumber() + ", " + locator.getColumnNumber() + ") : " + message;
			if (e != null) {
	...
	/**
	 * Returns an attributes list which contains all the attributes passed in, with
	 * any text of form "${xxx}" in an attribute value replaced by the appropriate
	...
	/**
	 * Return a new StringBuilder containing the same contents as the input buffer,
	 * except that data of form ${varname} have been replaced by the value of that
	 * var as defined in the system property.
	 */
