...
public abstract class AbstractEndpoint<S> {
	...
	/**
	 * The maximum number of headers in a request that are allowed. 100 by default.
	...
	public void init() throws Exception {
		...
		if (this.domain != null) {
			...
			ObjectName socketPropertiesOname = new ObjectName(
					domain + ":type=ThreadPool,name=\"" + getName() + "\",subType=SocketProperties");
	...
	/**
	 * Provides a common approach for sub-classes to handle exceptions where a delay
	 * is required to prevent a Thread from entering a tight loop which will consume
	 * CPU and may also trigger large amounts of logging. For example, this can
	 * happen with the Acceptor thread if the ulimit for open files is reached.
	 *
