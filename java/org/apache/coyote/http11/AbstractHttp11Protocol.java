...
public abstract class AbstractHttp11Protocol<S> extends AbstractProtocol<S> {
	...
	/**
	 * Will Tomcat accept an HTTP 1.1 request where the host header does not agree
	 * with the host specified (if any) in the request line?
	 *
	...
	/**
	 * If an HTTP request is received that contains an illegal header name (i.e. the
	 * header name is not a token) will the request be rejected (with a 400
	 * response) or will the illegal header be ignored.
	 *
	 * @return {@code true} if the request will be rejected or {@code false} if the
	...
	/**
	 * If an HTTP request is received that contains an illegal header name (i.e. the
	 * header name is not a token) should the request be rejected (with a 400
	 * response) or should the illegal header be ignored.
	 *
	 * @param rejectIllegalHeaderName {@code true} to reject requests with illegal
	 *                                header names, {@code false} to ignore the
	 *                                header
	 */
	...
	/**
	 * If true, the connectionUploadTimeout will be ignored and the regular socket
	 * timeout will be used for the full duration of the connection.
	 */
	...
	/**
	 * Regular expression that defines the User agents which should be restricted to
	 * HTTP/1.0 support.
	 */
	 ...
	 * normally means https is being used but can be used to fake https e.g behind a
	 * reverse proxy.
	 */
	...
	/**
	 * The protocols that are available via internal Tomcat support for access via
	 * HTTP upgrade.
	 */
	...
	protected Processor createProcessor() {
		Http11Processor processor = new Http11Processor(getMaxHttpHeaderSize(), getAllowHostHeaderMismatch(),
				getRejectIllegalHeaderName(), getEndpoint(), getMaxTrailerSize(), allowedTrailerHeaders,
				getMaxExtensionSize(), getMaxSwallowSize(), httpUpgradeProtocols, getSendReasonPhrase(),
