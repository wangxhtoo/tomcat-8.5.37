...
public class Http11Processor extends AbstractProcessor {
	...
	/**
	 * List of MIMES for which compression may be enabled. Note: This is not spelled
	 * correctly but can't be changed without breaking compatibility
	 */
	 ...
	 * Note that if {@link #server} is set, any application provided value will be
	 * over-ridden.
	 */
	...
	public Http11Processor(int maxHttpHeaderSize, boolean allowHostHeaderMismatch, boolean rejectIllegalHeaderName,
			AbstractEndpoint<?> endpoint, int maxTrailerSize, Set<String> allowedTrailerHeaders, int maxExtensionSize,
			int maxSwallowSize, Map<String, UpgradeProtocol> httpUpgradeProtocols, boolean sendReasonPhrase,
		...
		inputBuffer.addFilter(
				new ChunkedInputFilter(maxTrailerSize, allowedTrailerHeaders, maxExtensionSize, maxSwallowSize));
	 ...
	 *                    <code>off</code> or the minimum compression size in bytes
	 *                    which implies <code>on</code>
	 */
	 ...
	 *
	 * @param compressionMinSize The minimum content length required for compression
	 *                           in bytes
	 */
	 ...
	 * {@link Pattern}. e.g.: <code>gorilla|desesplorer|tigrus</code>.
	 *
	 * @param noCompressionUserAgents The regular expression for user agent strings
	 *                                for which compression should not be applied
	 */
	...
	/**
	 * Set compressible mime-type list (this method is best when used with a large
	 * number of connectors, where it would be better to have all of them referenced
	 * a single array).
	 *
	...
	/**
	 * Set the maximum number of Keep-Alive requests to allow. This is to safeguard
	 * from DoS attacks. Setting to a negative value disables the limit.
	 *
	...
	/**
	 * Get the maximum number of Keep-Alive requests allowed. A negative value means
	 * there is no limit.
	 *
	...
	/**
	 * Set the maximum size of a POST which will be buffered in SSL mode. When a
	 * POST is received where the security constraints require a client certificate,
	 * the POST body needs to be buffered while an SSL handshake takes place to
	...
	/**
	 * Set the flag to control whether a separate connection timeout is used during
	 * upload of a request body.
	 *
	...
	/**
	 * Check if compression should be used for this resource. Already checked that
	 * the resource could be compressed if the client supports it.
	 */
				...
				if (upgradeProtocol != null) {
					if (upgradeProtocol.accept(request)) {
						...
						InternalHttpUpgradeHandler upgradeHandler = upgradeProtocol
								.getInternalUpgradeHandler(getAdapter(), cloneRequest(request));
